...
const BurningBush = dynamic(
  () =>
    import('@/components/scenes/BurningBush').then((mod) => ({
      default: mod.BurningBush,
    })),
  { ssr: false }
)
const RedSea = dynamic(
  () =>
    import('@/components/scenes/RedSea').then((mod) => ({
      default: mod.RedSea,
    })),
  { ssr: false }
)
const GoliathScene = dynamic(
  () =>
    import('@/components/scenes/GoliathScene').then((mod) => ({
      default: mod.GoliathScene,
    })),
  { ssr: false }
)
const HarpScene = dynamic(
  () =>
    import('@/components/scenes/HarpScene').then((mod) => ({
      default: mod.HarpScene,
    })),
  { ssr: false }
)
const HealingScene = dynamic(
  () =>
    import('@/components/scenes/HealingScene').then((mod) => ({
      default: mod.HealingScene,
    })),
  { ssr: false }
)
const ResurrectionScene = dynamic(
  () =>
    import('@/components/scenes/ResurrectionScene').then((mod) => ({
      default: mod.ResurrectionScene,
    })),
  { ssr: false }
)
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <div
          className="absolute top-0 right-1/3 w-px h-24 bg-gradient-to-b from-blue-400/30 to-transparent animate-pulse"
          style={{ animationDelay: '1s' }}
        />
        <div
          className="absolute top-0 right-1/4 w-px h-40 bg-gradient-to-b from-purple-400/40 to-transparent animate-pulse"
